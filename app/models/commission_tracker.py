...
class CommissionTracker(db.Model):
    ...
    bank_application_id = db.Column(PGUUID(as_uuid=True), db.ForeignKey('bank_applications.id'), nullable=True)
    ...
    payoff_date = db.Column(DateTime(timezone=True), nullable=True)
    received_date = db.Column(DateTime(timezone=True), nullable=True)
